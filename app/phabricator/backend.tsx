...
function convertConduitRepoToRepoDetails(repo: ConduitRepo): Promise<PhabricatorRepoDetails | null> {
    return new Promise((resolve, reject) => {
        if (isExtension) {
            ...
            const callsignMappingsString = window.localStorage.getItem('PHABRICATOR_CALLSIGN_MAPPINGS')
            const callsignMappings = callsignMappingsString
                ? JSON.parse(callsignMappingsString)
