...
import { EMODENOTFOUND, fetchJumpURL, LSP } from '../backend/lsp'
...
export class TooltipPortal extends React.Component<Props, State> {
    ...
    private handleTooltips(): void {
        ...
        const hoverFetch = this.props.lsp.fetchHover(args).pipe(
            catchError(error => {
    ...
    private handleGoToDefinition = (defCtx: AbsoluteRepoFilePosition) => (e: MouseEvent) => {
        eventLogger.logCodeIntelligenceEvent()
        e.preventDefault()
        fetchJumpURL(this.props.lsp.fetchDefinition, defCtx).subscribe(defUrl => {
            if (!defUrl) {
                return
            }
